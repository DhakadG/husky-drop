  ...
  host.innerHTML = `<button class="mini" id="people-back" type="button">${icon("arrow-left", "ico-sm")} all people</button>
    <article class="person-profile rise" data-key="${escAttr(key)}">
      ...
      <div class="dashboard-grid">
        <section class="panel"><div class="section-title"><h2>${icon("download")} Drop links used</h2></div>${p.links.length ? `<ul class="person-links">${p.links.map((l) => `<li><button type="button" class="link-like" data-open-detail="${escAttr(l.slug)}" title="Open this drop link">${esc(l.label)}</button><small class="muted">${l.files ? `${l.files} files · ${fmtBytes(l.bytes)} · ` : ""}${l.events} events · ${ago(l.last)} ago</small></li>`).join("")}</ul>` : `<p class="muted">none</p>`}</section>
        <section class="panel"><div class="section-title"><h2>${icon("share-2")} Shares viewed</h2></div>${p.shares.length ? `<ul class="person-links">${p.shares.map((s) => `<li><button type="button" class="link-like" data-edit-share="${escAttr(s.slug)}" title="Open this share">${esc(s.label)}</button><small class="muted">${s.events} events · ${ago(s.last)} ago</small></li>`).join("")}</ul>` : `<p class="muted">none</p>`}</section>
      </div>
