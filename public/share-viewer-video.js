...
export function registerVideoContent(instance) {
  ...
  instance.on("contentLoad", (event) => {
    ...
    controls.innerHTML = `<button type="button" data-video-play aria-label="Play">${uiIcon("play", "pswp-video-control-icon")}</button><output>0:00 / 0:00</output><input type="range" min="0" max="0" step="0.01" value="0" aria-label="Video position" disabled><button type="button" data-video-mute aria-label="Mute">${uiIcon("volume-2", "pswp-video-control-icon")}</button>${previewUrl(file) ? `<button type="button" data-video-quality aria-pressed="false" aria-label="Play the original quality">HD</button>` : `<button type="button" data-video-quality class="hidden" aria-pressed="false">HD</button><span class="pswp-video-note" title="${file.previewState === "failed" ? "The streaming copy could not be made; playing the original." : "A 720p streaming copy is made overnight; until then this plays the original."}">${file.previewState === "failed" ? "original" : "optimising…"}</span>`}<button type="button" data-video-fullscreen aria-label="Enter video fullscreen">${uiIcon("maximize", "pswp-video-control-icon")}</button>`;
