...
export async function reportImageBatch(request, env, jobId) {
  ...
  for (const d of b.done || []) {
    ...
    if (!f) continue;
    job.items.push({ id: d.id, ok: true, newId: d.newId, size: Number(d.size) || 0, ms: Number(d.ms) || 0, via: cleanText(d.via || "", 12) || undefined, q: Number(d.q) || undefined });
    job.progress.done += 1;
...
export async function undoImageJob(env, jobId) {
  const jobs = await loadJobs(env);
  const job = jobs.find((j) => j.id === jobId);
  if (!job) return json({ error: "job not found" }, 404);
  ...
  if (!remaining) job.status = "undone";
  await saveJobs(env, jobs);
