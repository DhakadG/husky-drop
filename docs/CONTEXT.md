...
| `admin-workflow-test.mjs`, `kv-budget-test.mjs`, `exif-test.mjs`, `transcode-previews-test.mjs`, `identity-ban-test.mjs`, `image-rules-test.mjs`, `image-archive-order-test.mjs`, `timeseries-test.mjs`, `preflight-index-test.mjs`, `completion-flush-test.mjs`, `revoke-retry-test.mjs`, `folder-list-test.mjs`, `share-edit-index-test.mjs`, `preview-report-test.mjs`, `share-preview-put-test.mjs`, `share-roots-test.mjs`, `pin-counter-test.mjs`, `media-epoch-test.mjs`, `image-undo-test.mjs`, `live-dismiss-test.mjs`, `share-index-jobs-test.mjs`, `activity-person-key-test.mjs`, `preview-crawl-test.mjs`, `stitch-test.mjs`, `promise-audit-test.mjs`, `image-idempotency-test.mjs` | admin flows, KV write budget, EXIF, transcoder API, ban ids keep their case, pausing a rule keeps its recipe, the archive keeps HDR originals and never moves one before its replacement is stored, the chart keeps drop and share traffic apart, preflight sees every past completion, a failed completion flush still counts each file once, a failed Drive revoke is kept and retried, the folder picker pages past 100, a gallery folder change starts an index job, parallel video shard reports and image-preview PUTs are never lost, folders removed from a share leave its index, wrong PINs cost no KV writes until a lockout, removing a folder retires its media URLs, a failed undo is reported and retryable, a dismissed live session stays dismissed and a dead one is listed as stopped, a finished index job stays finished, Activity resolves people like the People tab, the coverage crawl is bounded and reports what it skipped, the matching model never sees e-mails or device ids, no browser promise that can reject is left unhandled and blocked storage cannot crash a page, an image job never stores or counts a file twice |
